...
- 2026-02-25: Move SARB artifacts from tools/SARB to docs/sarb - SARB guidance is documentation; keeps review artifacts with canonical docs and reduces ambiguity about what "tools" means.
...
- 2026-02-27: Treat CHR QEMU guest-agent IP detection as non-blocking/non-dependency for macOS UTM feature delivery - Direct QEMU + control validation shows CHR non-response on macOS Intel/QEMU 10.2.1 (HVF) despite documented support claims; VM management remains in scope while IP setup uses fallback manual/serial/config-hint workflow.
